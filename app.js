        ...
        *****************************************************/
        document.addEventListener('DOMContentLoaded', () => {
            ...
            if (openCVBtn && cvOverlay) {
                openCVBtn.addEventListener('click', () => {
                    ...
                    initializeParticles(); // Start particles when overlay opens
                });
            }

            [closeIcon, returnBtn].forEach(btn => {
            ...
            function initializeParticles() {
                ...
                class Particle {
                    ...
                    update() {
                        ...
                        // Interactive size based on mouse position
                        const dx = this.x - mouseX;
                        const dy = this.y - mouseY;
                        const distance = Math.sqrt(dx * dx + dy * dy);
                        this.size = Math.max(this.baseSize, 5 - (distance * 0.01));
                    }
            ...
            const messageParser = {
                parseLinks: text => text.replace(
                    /(https?:\/\/[^\s]+)/g, 
                    url => `<a href="${url}" target="_blank" class="neon-link">${url}</a>`
                ),
                parseLists: text => text.replace(/- (.*?)(\n|$)/g, '<li>$1</li>'),
                formatResponse: text => text.replace(/\n/g, '<br>')
            };
            ...
            function addMessage(content, isUser = false) {
                ...
                const processedContent = messageParser.formatResponse(
                    messageParser.parseLists(
                        messageParser.parseLinks(content)
                    )
                );
                ...
                messageDiv.innerHTML = `
                    ...
                    <div class="ai-message-header">
                        <img src="assets/images/ai_avatar.png" class="ai-mini-avatar">
                ...
                updateHistory(processedContent, isUser);
                scrollChatSmoothly();
            }

            // Chat Utilities
            function updateHistory(message, isUser) {
            ...
            // Input Handling
            aiQuery.addEventListener('input', (e) => {
                if (e.target.value.length > 200) {
                    e.target.value = e.target.value.slice(0, 200);
                    addMessage("Message truncated to 200 characters", false);
                }
            });
        ...
        const AI = {
            ...
            processQuery(input) {
                ...
                const intent = Object.entries(this.intents).find(([name, data]) => 
                    name !== 'fallback' && 
                    data.patterns.some(pattern => cleanInput.includes(pattern))
                );
                
                return intent?.[1].responses[Math.floor(Math.random() * intent[1].responses.length)] ||
                    this.intents.fallback.responses[
                        Math.floor(Math.random() * this.intents.fallback.responses.length)
                    ];
            }
        };
